...
def test_non_tty_prints_path_and_skips_editor(isolated_home, monkeypatch, capsys):
    force_tty(monkeypatch, False)

    def explode(*a, **k):  # editor must not be invoked
        raise AssertionError("editor should not run in non-tty mode")

    monkeypatch.setattr("aicommit.commands.config.subprocess.run", explode)
    rc = run()
...
def test_editor_not_found_returns_127(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("aicommit.commands.config.subprocess.run", raise_not_found)
